...
const TileSelector = ({ onSelectTile }) => {
  const { tileCollections, selectedCategory, setSelectedCategory, setSelectedBorder } = useTileSimulator();
...
const handleTileClick = (tile) => {
  ...
  return (
    <section className="w-full h-auto p-1">
      <h4 className="text-md  mb-2 font-light font-poppins text-center lg:text-left uppercase tracking-wide">(SCROLL FOR MORE OPTIONS)</h4>
      <div className="flex flex-col md:flex-row gap-2">
        <div className="flex md:flex-col justify-start items-start custom-scrollbar overflow-x-auto md:overflow-y-auto gap-2 md:min-w-[180px] bg-gray-100 p-2 rounded-lg">
          {Object.keys(tileCollections).map((collection) => (
            <button
              ...
              className={`px-4 py-2 whitespace-nowrap font-poppins font-light ${selectedCategory === collection
        ...
        <div className="grid grid-cols-3  lg:grid-cols-2  gap-2 flex-1 bg-gray-100 p-2 rounded-lg font-light font-poppins overflow-x-auto custom-scrollbar lg:overflow-y-auto">
          {tileCollections[selectedCategory ].map((tile) => (
            <div
              key={tile.id}
              className={`relative aspect-square group cursor-pointer ${selectedTile?.id === tile.id ? 'overflow-hidden' : ''
                }`}
              onClick={() => {
